...
export default function PersonalTodoCard({ todo }: { todo: PersonalTodo }) {
  ...
  return (
    <article
      ...
      className="group flex shrink-0 cursor-grab flex-col gap-3 rounded-xl border border-line bg-panel px-4 pb-5 pt-4 shadow-sm transition hover:-translate-y-0.5 hover:border-teal-accent/50 hover:shadow-md active:cursor-grabbing"
    >
      <div className="flex items-start gap-2">
        {editingTitle ? (
          ...
          <button
            ...
            title="Click to edit"
            className="min-w-0 flex-1 cursor-text rounded-md px-1 py-0.5 text-left text-sm font-semibold text-deep transition hover:text-teal-accent"
          >
            {todo.title}
          </button>
        )}
